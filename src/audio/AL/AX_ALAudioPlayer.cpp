...
ALAudioPlayer::ALAudioPlayer(std::size_t maxSources) {
    m_sources.resize(maxSources);
}

ALAudioPlayer::~ALAudioPlayer() {}

ALAudioPlayer::SourceWrapper* ALAudioPlayer::AcquireFreeSource() {
...
void ALAudioPlayer::PlaySound(ALAudioBuffer& buffer) {
...
void ALAudioPlayer::SetLooping(bool loop) {
    m_looping = loop;
...
void ALAudioPlayer::SetGain(float gain) {
    m_gain = gain;
    for (auto& w : m_sources) w.source.SetGain(gain);
}

void ALAudioPlayer::SetPosition(float x, float y, float z) {
    m_pos[0] = x; m_pos[1] = y; m_pos[2] = z;
    for (auto& w : m_sources) w.source.SetPosition(x, y, z);
}

void ALAudioPlayer::SetVelocity(float x, float y, float z) {
