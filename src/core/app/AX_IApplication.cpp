...
SharedState::SharedState(ApplicationSpecification spec, uint32_t maxEventsQueue)
...
CursorMode SharedState::GetCursorMode() const {
    std::lock_guard<std::mutex> lock(m_Mutex);
    return m_CursorMode;
}
...
void SharedState::PushEvent(const Event& event) {
    std::lock_guard<std::mutex> lock(m_Mutex);
    if (event.type == EventType::Void) return;
...
void SharedState::SetCursorMode(CursorMode mode) {
    std::lock_guard<std::mutex> lock(m_Mutex);
    m_CursorMode = mode;
}
