...
void GenericThreadContext::StartCycle(int64_t mssleep, std::function<std::unique_ptr<AXGEmpty>()> initFunc, std::function<void(GenericThreadContext& gctx)> subCycle) {
...
void ApplicationThreadContext::StartApp(std::function<std::unique_ptr<IApplication>()> initFunc) {
...
void RenderThreadContext::StartGraphics(std::function<std::unique_ptr<IRenderContext>()> initFunc) {
