...
struct SamplerDesc {
    float maxAnisotropy{0.0f}; // anisotropic filtering, 0.0 means disabled
...
using SamplerHandle = ExternalHandle<SamplerTag>;
...
struct TextureDesc {
    ...
    // Only used when UpdateTexture is called, Updating for cubemaps is "per-face"; unlike 2d-arrays and 3
    TextureCubemapFace updateThisCubemapFace{TextureCubemapFace::NegativeZ};

    // Optional texture data
    utils::CowSpan<utils::URaw> initialData{}; // pixels data: for tex 2d: One single element, 2d-arrays/3d: One single element but aligned, for cubemap: 6
};
...
struct ResourceHandle {
    ...
    ResourceHandle() : index(UINT32_MAX), generation(UINT32_MAX) {}
...
struct ResourceBinding {
    // Used only when the binding is created or updated.
    // Backend resolves this to a __shader_res_id.
    ...
    // Buffer parameters for UBO/SSBO
    uint64_t offset{0}; 
    uint64_t range{0}; // 0 = whole resource

    // Optional sampler state for Texture resources.
    SamplerHandle sampler{utils::INVALID_HANDLE};

    uint32_t __shader_res_id{0}; // AUTO-GENERATED BY BACKEND, DO NOT CHANGE
};
